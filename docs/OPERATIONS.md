...
| `DEEVY_AGENT_HARNESS`             | `claude-code`            | Nothing: which coding-agent CLI works the Runs. The runtime refuses to start on a name it does not know, or a binary it cannot run.                           |
| `ANTHROPIC_API_KEY`               | —                        | With `claude-code`, every session fails at once unless the CLI is signed in some other way. Read by Claude Code, not by the runtime.                          |
