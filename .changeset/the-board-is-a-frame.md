...
A Board is now a frame rather than a page that grows. On anything wider than a phone it is exactly as tall as the room below the filters, and each column scrolls its own cards: the Workflow stays on screen while you read down a State, the column headers and their counts stay put, and a wheel over a column does what a wheel does everywhere else. A Board on a phone still grows and scrolls with the page, because the chrome above it leaves about 300px there and a frame that size would show two cards.
