// forked from design-sync lib/story-imports.mjs - skip rule-2 resolution for imports made from inside node_modules (lucide-react's 1,700 icon modules cost a plugin round-trip each: 30s per preview)
  ...
  const policyRedirect = {
    ...
    setup(b) {
      b.onResolve({ filter: /.*/ }, async (a) => {
        ...
        // FORK: a module inside node_modules never imports a preview or a package source file - skip the round-trip.
        if (a.importer && a.importer.includes('/node_modules/')) return null;
